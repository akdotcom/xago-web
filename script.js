document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.Worker) {
        ...
        aiWorker.onerror = function(error) {
            console.error('AI Worker Error:', error.message, error);
            // Handle worker error, perhaps by stopping AI or notifying the user
            player2HandContainer.classList.remove('ai-thinking-pulse');
        };
    } else {
        console.warn('Web Workers not supported in this browser. AI will run on the main thread.');
        // Fallback or error message if workers are not supported
    }
...
function animateView() {
    ...
    // --- AI Player Logic (now primarily handled by Web Worker) ---

    // Helper function to convert HexTile instances to plain data objects for the worker
    function tileInstanceToPlainData(tile) {
        if (!tile) return null;
        return {
            id: tile.id,
            playerId: tile.playerId,
            edges: [...tile.edges], // Ensure a copy
            orientation: tile.orientation,
            x: tile.x,
            y: tile.y
            // Do not include methods or complex objects that cannot be cloned.
        };
    }

    // Helper function to convert an array of HexTile instances to plain data
    function handToPlainData(hand) {
        return hand.map(tileInstanceToPlainData);
    }

    // Helper function to convert boardState (HexTile instances) to plain data
    function boardStateToPlainData(currentBoardState) {
        const plainBoardState = {};
        for (const key in currentBoardState) {
            plainBoardState[key] = tileInstanceToPlainData(currentBoardState[key]);
        }
        return plainBoardState;
    }


    function performAiMove() {
        if (currentPlayer !== 2 || player2Hand.length === 0) {
            console.log("Main: AI not to move (not turn, or no tiles).");
            player2HandContainer.classList.remove('ai-thinking-pulse');
            return;
        }
        if (opponentType === 'human') {
            console.log("Main: Opponent is human, AI will not move.");
            player2HandContainer.classList.remove('ai-thinking-pulse');
            return;
        }
        if (!aiWorker) {
            console.error("Main: AI Worker not initialized. Cannot perform AI move.");
            player2HandContainer.classList.remove('ai-thinking-pulse');
            // Optionally, fall back to main thread AI or switch turn.
            switchTurn();
            return;
        }

        console.log("Main: Player 2 (AI) is thinking... Posting task to worker.");
        // Ensure ai-thinking-pulse is added if not already (e.g. by switchTurn)
        player2HandContainer.classList.add('ai-thinking-pulse');


        const messagePayload = {
            taskType: 'move',
            boardState: boardStateToPlainData(boardState),
            playerHand: handToPlainData(player2Hand),
            opponentHand: handToPlainData(player1Hand), // Opponent's hand for smarter AI
            currentPlayerId: currentPlayer, // Should be 2
            opponentType: opponentType,
            // Pass any other relevant state like NUM_TILES_PER_PLAYER if needed by worker's evaluateBoard
            // For evaluateBoard's initial penalty:
            numTilesPerPlayer: NUM_TILES_PER_PLAYER,
            player1HandSize: player1Hand.length // Pass current sizes for win/loss checks in worker
        };
        aiWorker.postMessage(messagePayload);
    }

    function performAiTileRemoval() {
        if (currentPlayer !== 2 || !isRemovingTiles || currentSurroundedTilesForRemoval.length === 0) {
            console.log("Main: AI not to remove (conditions not met).");
            player2HandContainer.classList.remove('ai-thinking-pulse');
            return;
        }
        if (opponentType === 'human') {
            console.log("Main: Opponent is human, AI will not remove tile.");
            player2HandContainer.classList.remove('ai-thinking-pulse');
            return;
        }
        if (!aiWorker) {
            console.error("Main: AI Worker not initialized. Cannot perform AI tile removal.");
            player2HandContainer.classList.remove('ai-thinking-pulse');
            // Optionally, fall back or switch turn.
            isRemovingTiles = false; // To prevent getting stuck
            currentSurroundedTilesForRemoval = [];
            switchTurn();
            return;
        }

        console.log(`Main: Player 2 (AI - ${opponentType}) is choosing a tile to remove... Posting task to worker.`);
        // Ensure ai-thinking-pulse is added
        player2HandContainer.classList.add('ai-thinking-pulse');

        const messagePayload = {
            taskType: 'remove',
            boardState: boardStateToPlainData(boardState),
            currentSurroundedTilesForRemoval: handToPlainData(currentSurroundedTilesForRemoval), // Send plain data
            currentPlayerId: currentPlayer, // Should be 2
            opponentType: opponentType
        };
        aiWorker.postMessage(messagePayload);
    ...
    // --- Minimax AI Helper Functions (These were moved to ai_worker.js) ---
    // getAllPossibleMoves, evaluateBoard, simulateRemovalCycle, findBestMoveMinimax, deepCopyBoardState (for HexTile instances)
    // calculateScoresForBoard (if used by evaluateBoard directly on main thread, otherwise it's also in worker)
    // Note: deepCopyBoardState on the main thread is still useful for its own simulations if any,
    // but the worker will have its own version for its internal data.
    // calculateScoresForBoard is kept on main for direct score calculation for display.

    // Helper function to deep copy board state with HexTile instances (remains on main thread for non-worker logic)
    ...
    // evaluateBoard and other complex AI logic functions are now in ai_worker.js.
    // Only stubs or simplified versions might remain if needed for non-AI UI logic.

    if (aiWorker) {
        aiWorker.onmessage = function(event) {
            console.log("Main: Received message from AI Worker:", event.data);
            player2HandContainer.classList.remove('ai-thinking-pulse'); // Stop pulsing

            const { taskType, move: bestMoveData, tileToRemoveData } = event.data;

            if (taskType === 'moveResult') {
                if (bestMoveData && bestMoveData.tile) {
                    const tileToPlaceInstance = player2Hand.find(t => t.id === bestMoveData.tile.id);

                    if (!tileToPlaceInstance) {
                        console.error(`Main: AI Error - Best move tile (ID: ${bestMoveData.tile.id}) not found in player 2 hand!`);
                        switchTurn(); // Pass turn
                        return;
                    }
                    tileToPlaceInstance.orientation = bestMoveData.orientation;

                    console.log(`Main: AI (${opponentType}) attempting to place tile ${tileToPlaceInstance.id} at (${bestMoveData.x}, ${bestMoveData.y}) with orientation ${tileToPlaceInstance.orientation}`);
                    if (placeTileOnBoard(tileToPlaceInstance, bestMoveData.x, bestMoveData.y)) {
                        player2Hand = player2Hand.filter(t => t.id !== tileToPlaceInstance.id);
                        displayPlayerHand(2, player2Hand, player2HandDisplay);
                        console.log(`Main: AI (${opponentType}) successfully placed tile ${tileToPlaceInstance.id}.`);

                        checkForSurroundedTilesAndProceed();
                        updateViewParameters();
                        animateView();
                    } else {
                        console.error(`Main: AI (${opponentType}) failed to place tile ${tileToPlaceInstance.id} despite worker considering it valid.`);
                        switchTurn();
                    }
                } else {
                    console.log(`Main: AI (${opponentType}) could not find any valid move (worker passed). Passing turn.`);
                    calculateScores();
                    switchTurn();
                }
            } else if (taskType === 'removeResult') {
                if (tileToRemoveData) {
                    // Find the actual tile instance in boardState using coordinates from tileToRemoveData
                    const tileToRemoveInstance = boardState[`${tileToRemoveData.x},${tileToRemoveData.y}`];

                    if (tileToRemoveInstance && tileToRemoveInstance.id === tileToRemoveData.id) {
                        console.log(`Main: AI (${opponentType}) decided to remove tile ${tileToRemoveInstance.id} at (${tileToRemoveInstance.x}, ${tileToRemoveInstance.y})`);
                        // removeTileFromBoardAndReturnToHand will handle further logic, including re-triggering AI removal if necessary
                        removeTileFromBoardAndReturnToHand(tileToRemoveInstance);
                    } else {
                        console.error("Main: AI Worker chose a tile for removal that doesn't match current board state:", tileToRemoveData);
                        // Fallback: Exit removal mode to prevent getting stuck.
                        isRemovingTiles = false;
                        currentSurroundedTilesForRemoval = [];
                    ...
                    console.error("Main: AI Worker failed to choose a tile for removal or indicated no tile to remove.");
